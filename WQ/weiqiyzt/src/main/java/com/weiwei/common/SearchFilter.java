...
public class SearchFilter {
	public enum Operator {
		EQ,NEQ, LIKE, CONTAIN, STARTWITH, ENDWITH, GT, LT, GTE, LTE, IN, OEQ, IS, NIS
	}
		...
		public static Object[] convert(String[] strArray, Type type,
			...
			for (int i = 0, len = strArray.length; i < len; i++) {
				...
				case Date:
					...
					break;
				case Timestamp:
					list.add(new java.sql.Timestamp(DateTime.parse(str,format)
							.getMillis()));
					break;
				default:
					list.add(str);
				...
				if (CollectionUtils.isNotEmpty(filters)) {
					...
					for (SearchFilter filter : filters) {
						...
						case IN:
							...
							break;
						case OEQ:
							predicates.add(builder.or(builder.equal(exp, filter.value[0])));
							break;
						case IS:
